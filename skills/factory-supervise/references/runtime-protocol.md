...
`.factory/FACTORY.json` holds portable machine configuration. Project and domain prose live in `.factory/CONTEXT.md`.
...
`.factory/db/state.sqlite` is authoritative for all active and completed tickets. Tickets are rows keyed by ID. Concurrent tickets do not overwrite one another. `fstate status` reconstructs this nested JSON view for agents. Dashboards query the database instead of parsing JSON files.
...
Specialist agents do not edit state. The supervisor is the semantic writer. Every mutation goes through:
...
Every successful mutation increments `revision`, updates the top-level and affected ticket `updatedAt` values, and updates the narrower task, session, message, or usage timestamp when applicable. Timestamps are UTC RFC 3339. `message` is the latest meaningful update, not a transcript. When blocked, `blocker` is an object containing `reason`, `since`, `owner` (`user`, `agent`, or `external`), and optional `task`. Unblock clears it.
...
- Opens `.factory/db/state.sqlite` read-only. It does not write. WAL sidecars stay in `.factory/db/`.
- Treats `meta.revision` as the change cursor and `meta.schemaVersion` as the compatibility boundary.
- Renders each ticket independently from `stage`, `status`, `current_task`, task review summary, `message`, `blockers`, and timestamps.
...
`pi-herdr-subagents` applies the supplied subagent `name` as the pane label. Display tokens are reported with one call per value:
...
`task` and `round` are omitted or cleared when they do not apply. `report-agent` is not used for these fields. The Pi integration and Herdr own lifecycle state. This metadata is display-only. Token names are available to Herdr sidebar rows as `$ticket`, `$stage`, `$task`, and `$round`.

## Subagent tools

`pi-herdr-subagents` provides `subagent`, `subagent_resume`, `subagent_interrupt`, and `subagents_list`.

`subagent` accepts `name`, `model`, `thinking`, `skills`, `tools`, and `cwd`. `thinking` defaults to `medium` when omitted from `FACTORY.json`. Omitting `thinking` on the tool call makes the child inherit the parent session's level. `subagent_resume` does not take `thinking`.

Review tools are `read`, `grep`, `find`, and `ls`. `bash`, `edit`, and `write` are not review tools.

The spawn result includes `sessionFile`, Pi session ID, pane ID, pane name, and role.

`scripts/pi-session-reader.py contract --schema <factory.plan.v3|factory.work.v3|factory.review.v4|factory.wrapup.v1> --check` is the stage gate against `sessionFile`. Exit 0 is valid. Exit 3 or 4 is invalid. Compact JSON is printed when `--check` is omitted. The reader does not treat steered prose as the contract and does not parse Pi JSONL with inline Python.

`scripts/review-packet.py` prints `packet:` and `diff:` paths. The diff is the cumulative uncommitted ticket worktree change.

`caller_ping` is a child request for user or supervisor input. It is not a failure.

v0 state stores verdict, round count, usage, and unresolved blockers. Detailed findings remain in Pi sessions.

## Usage collection

Stored usage values are numbers, not display strings such as `in 28, out 6.9k`. `cacheHit` is not persisted. It is derived for display only when its formula is defined.

The completion steer from current `pi-herdr-subagents` may include `details.contextUsage` with `tokens`, `contextWindow`, and `percent`. That object is current context occupancy, not cumulative model usage or billed tokens. It maps to the matching session's optional `context` object, with `contextWindow` stored as `window`. It is not copied into `usage.tokens.total`.

Billed usage is one read of the session file:

```text
python3 <skill-dir>/scripts/pi-session-reader.py usage <session-file>
```

That JSON is persisted with:

```text
node <skill-dir>/scripts/fstate/cli.mjs usage record --ticket <ticket-id> --stage <stage> --session <session-file> [--task <task-id>] [--round <n>] --expected-revision <revision>
```

`usage record` calls this reader. A later `fstate` may call Pi's session statistics API instead.

The reader walks the active `parentId` branch of the append-only Pi JSONL and sums Pi's numeric usage records, including assistant messages and any compaction or branch-summary entries Pi counts in its own session totals. Pi's reported `totalTokens` is preserved when available. `cost.total` is stored as `costUsd`. Prices are not estimated locally. If a later subagent extension version adds cumulative numeric usage to the structured steer, that payload is preferred after its version and session identity are validated.

Usage is keyed by Pi session ID. Recording is an idempotent replacement of that session's cumulative values through `throughEntryId`, not an increment. A resumed session therefore updates one record instead of double-counting earlier turns. `fstate usage show` derives per-stage, per-task, per-model, per-ticket, and total summaries from these records.
...
An idle or closed session is not proof that plan, work, review, or wrap-up succeeded.
